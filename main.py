...
class RateLimiter:
    def __init__(self):
        self.last = 0
        self.delay = 0.6  # Increased for stability
...
class Clone:
        ...
        for role_data in roles_data:
            ...
            if name == "@everyone":
                try:
                    ...
                    await guild_to.default_role.edit(permissions=perms)
                    self.bot.role_map[role_data.get("id")] = guild_to.default_role
                    await self.log_add("updated @everyone")
                except Exception as e:
    ...
    async def categories_create(self, guild_to, channels_data):
        ...
        console.print(f"[dim]Creating {self.total} categories...[/dim]")
        ...
        for cat_data in cats_data:
            ...
            try:
                ...
                for ow in cat_data.get("permission_overwrites", []):
                    role_id = self.safe_int(ow.get("id"), 0)
                    role = self.bot.role_map.get(role_id)
                    if role:
                        ...
                        overwrites_to[role] = discord.PermissionOverwrite.from_pair(allow, deny)
    ...
    async def channels_create(self, guild_to, channels_data):
        ...
        console.print(f"[dim]Creating {self.total} channels...[/dim]")
...
class CloneBot:
    ...
    async def do_full_clone(self):
        ...
        # Execute operations sequentially with clear separation
